...
import com.os13musicapp.os13musicplayer.fragment.main.LibraryFragment
import com.os13musicapp.os13musicplayer.fragment.main.SearchFragment
import com.os13musicapp.os13musicplayer.fragment.main.FavoriteFragment
...
class MainActivity : AppCompatActivity() {
    ...
    private fun ivControl() {
        iv_play_pause_collapsed.setOnClickListener {
            pausePlay()
        }
        iv_play_pause_expanded.setOnClickListener { pausePlay() }
        iv_next_collapsed.setOnClickListener {
            next()
            startIntentService("next")
        }
        iv_next_expanded.setOnClickListener {
            next()
            startIntentService("next")
        }
        iv_back_expanded.setOnClickListener {
            back()
            startIntentService("back")
        }
        rl_shuffle.setOnClickListener {
            shuffleCheck()

        }
        rl_repeat.setOnClickListener {
            repeatCheck()
        }
    ...
    private fun favoriteCheck() {
        if (arrayList[sharedPreferences!!.getInt("pos", 0)].favorite == 0) {
            ...
            arrayList[sharedPreferences!!.getInt("pos", 0)].favorite = 1
        } else {
            ...
            arrayList[sharedPreferences!!.getInt("pos", 0)].favorite = 0
        }
    }
    ...
    private fun repeatCheck() {
        when {
            sharedPreferences?.getInt("repeat", 0) == 0 -> {
                rl_repeat.setBackgroundDrawable(this.resources.getDrawable(R.drawable.bg_shufffle_true));
            ...
            sharedPreferences?.getInt("repeat", 0) == 1 -> {
                rl_repeat.setBackgroundDrawable(this.resources.getDrawable(R.drawable.bg_shufffle_true))
                iv_repeat.setImageDrawable(resources.getDrawable(R.drawable.repeat_true))
                tv_repeat.setTextColor(Color.parseColor("#ffffff"))
                ...
                rl_repeat.setBackgroundDrawable(this.resources.getDrawable(R.drawable.bg_shuffle_false))
    ...
    private fun shuffleCheck() {
        if (sharedPreferences?.getBoolean("shuffle", false) == false) {
            rl_shuffle.setBackgroundDrawable(this.resources.getDrawable(R.drawable.bg_shufffle_true))
            ...
            rl_shuffle.setBackgroundDrawable(this.resources.getDrawable(R.drawable.bg_shuffle_false))
    ...
    fun updateDB() {
        ...
        if (sharedPreferences?.getBoolean("shuffle", false) == true) {
            ...
            arrayList.shuffle(Random(sharedPreferences!!.getLong("seed", 0)))
            for (i in arrayList.indices) {
                if (arrayList[i].path == sharedPreferences?.getString("path", "")) {
                    edit!!.putInt("pos", i)
                    boolean = true
                    edit!!.apply()
                }
            ...
            if (sharedPreferences?.getString("array", "").equals("song")) {
                arrayList = dbSong.getSong()
                val position = dbSong.getPositionSong(sharedPreferences?.getString("path", "")!!)
...
//        val seconds: Int
        ...
        tv_sb_end.text = "$minutes:$secondsString"
...
//        transaction.addToBackStack(null)
    ...
    private fun startIntentService(status: String) {
...
//            doStart()
    ...
    fun setSound() {
        ...
        sb_sound.max = audioManager!!.getStreamMaxVolume(AudioManager.STREAM_MUSIC);
        sb_sound.progress = audioManager!!.getStreamVolume(AudioManager.STREAM_MUSIC)
    ...
    private var broadcastReceiver: BroadcastReceiver = object : BroadcastReceiver() {
        ...
        override fun onReceive(context: Context, intent: Intent) {
            ...
            if (action.equals("SERVICE_BACK", ignoreCase = true)) {
                playBC()
                updateDB()
                val song = arrayList[sharedPreferences!!.getInt("pos", 0)]
    ...
    fun doStart() {
        sb_duration.max = sharedPreferences!!.getLong("time", 0).toInt()
    ...
    private val update = object : Runnable {
        override fun run() {
            if (mp != null) {
                val currentTime = mp?.currentPosition!!.toLong()
                sb_duration.progress = currentTime.toInt()
    ...
    private fun sendBrIvPlay() {
        val intent = Intent("IV_PLAY")
        sendBroadcast(intent)
    }

    private fun sendBrShuffle(action: String) {
        ...
        if (sharedPreferences?.getInt("repeat", 0) != 2) {
            if (sharedPreferences!!.getInt("pos", 0) + 1 == arrayList.size) {
                ...
                if (sharedPreferences!!.getInt("pos", 0) + 1 > arrayList.size) {
                    ...
                    song = arrayList[sharedPreferences!!.getInt("pos", 0) + 1]
                    edit!!.putBoolean("isplay", true)
                }
            }
        } else {
            song = arrayList[sharedPreferences!!.getInt("pos", 0)]
        }
...
//                        arrayList = arrSongPlaylist
        ...
        updateView(sharedPreferences?.getString("art", "")!!, name!!, sharedPreferences?.getString("artist", "")!!,
                sharedPreferences?.getLong("time", 0)!!, arrayList[sharedPreferences!!.getInt("pos", 0)].favorite)

    }


    @SuppressLint("Recycle")
    private fun listOfSongs(context: Context?): ArrayList<Song> {
